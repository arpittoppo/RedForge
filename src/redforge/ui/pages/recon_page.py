...
class ReconPage(QWidget):
    ...
    def __init__(
        ...
        self.recon_entries_label = QLabel(
            "Recon Entries"
        )
        ...
        self.new_entry_button.clicked.connect(
            self._create_recon
        )
        self.delete_entry_button.clicked.connect(
          self._delete_recon
        )

        self.recon_list.itemClicked.connect(
            self._select_recon
        )
    ...
    def _load_recons(
        ...
        recons = self.recon_service.get_recons(
            self.engagement.id
        )
        ...
        for recon in recons:
            ...
            item = QListWidgetItem(
                f"[{recon.recon_type}] {recon.value}"
            )
    ...
    def _create_recon(
        ...
        recon = self.recon_service.create_recon(
            self.engagement.id
        )

        self._load_recons()

        self._select_recon_by_id(
            recon.id
        )
    ...
    self,
     ...
     """
    Delete the selected recon entry.
    """

     if self.recon is None:
        return

     self.recon_service.delete_recon(
        self.recon
    )
     ...
     if self.recon_list.count() > 0:
        ...
        self.recon_list.setCurrentItem(
            item
        )
    ...
    def _select_recon_by_id(
        self,
        recon_id: int,
    ):
        """
        Select a recon entry by its ID.
        """

        for index in range(
            self.recon_list.count()
        ):

            item = self.recon_list.item(
                index
            )
    ...
    def _select_recon(
        ...
        self.recon = self.recon_service.get_recon(
            recon_id
        )
    ...
    def _save_recon(
        ...
        if (
            recon_type == self.recon.recon_type
            and
            value == self.recon.value
        ):
            return
